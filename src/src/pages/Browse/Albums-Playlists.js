...
import AlbumPlaylist from "../../components/Browse/Album-Playlist/Album-Playlist";
import {getKey} from "../../functions/uniqueSuffix";

import {deleteJunkCache} from "../../react-query-client";

function Albums_Playlists({
  token,
  ...
  const {data, error, isError, isSuccess} = useQuery(
    ...
    () => {
      return fetcher(
        {
          url: `https://antara.onrender.com/api/v1/browse?id=${browseId}${
    ...
    <Fragment>
      ...
      <div>
        <div className="flex-align">
          <Skeleton
            className="skeleton skeleton-bg skeleton-thumbnail-sbox"
            animation="wave"
          />
...
export default /*memo*/ Albums_Playlists;
